...
const csvData = new SharedArray('another data name', function () {
  return open('./Shipping_Attempt_2.csv')
    .split('\n')
...
export const options = {
  scenarios: {
    default: {
      ...
      vus: 1,
      duration: '15m', // Increase the total test duration
    },
  },
  thresholds: {
    checks: ['rate==1.0'],
...
export default function () {
  group('Testing PT Console API', function () {
    ...
    const JWT_token = {
      headers: {
        Authorization:
          'Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJyLnNoYWRtYW5AYmppdGdyb3VwLmNvbSIsImV4cCI6MTc0ODYwMTg3OCwiaWF0IjoxNzQ4NTE1NDc4fQ.BbjSeacSfcu5IyW_Xs4nQTWuPQwRy7ao4FqSfl0a3Mk'
      },
      timeout: '6000000000ms'
    };

    const formData = {
      csv_file: http.file(payload, `SP36 importing 6K devices US_VUs 1 using k6 testing tool ${currentTime} Tanvir`, 'text/csv'),
    };
...
export function handleSummary(data) {
  ...
  return {
    [`${outputDirectory}/SP36_html_report_6K_devices_US_VUs 1_${currentTime} Tanvir.html`]: htmlReport(data),
  };
}
