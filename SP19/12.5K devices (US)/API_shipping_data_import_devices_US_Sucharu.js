...
const csvData = new SharedArray('another data name', function () {
  return open('./SP19_T4 CW Tanvir v4.csv')
    .split('\n')
...
export const options = {
  scenarios: {
    default: {
      ...
      vus: 1,
      duration: '22m', // Increase the total test duration
    },
  },
  thresholds: {
    checks: ['rate==1.0'],
...
export default function () {
  group('Testing PT Console API', function () {
    ...
    const JWT_token = {
      headers: {
        Authorization:
          'Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJzdWNoYXJ1LnN1bmlwb29uQGJqaXRncm91cC5jb20iLCJleHAiOjE3Mjc5NDIzNTYsImlhdCI6MTcyNzg1NTk1Nn0.fREPca5uVksr-IPodvnHZiY3-oVYhYvtKg_oZPAKvuw'
      },
      timeout: '6000000000ms'
    };

    const formData = {
      csv_file: http.file(payload, `SP19 importing 12.5K devices US_VUs 4 using k6 testing tool ${currentTime} Sucharu`, 'text/csv'),
    };

    group('Checking shipping device import', function () {
      let startTime = new Date().getTime(); // Record start time
      let res = http.post(
        'https://cw-consolebo.global.qa.pocketalk.com/iotconsoleapi-global-bo/api/private/shipping-data/',
        formData,
        JWT_token
      );
...
export function handleSummary(data) {
  ...
  return {
    [`${outputDirectory}/SP19_html_report_12.5K_devices_US_VUs 4_${currentTime} Sucharu.html`]: htmlReport(data),
  };
}
