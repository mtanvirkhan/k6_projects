...
const csvData = new SharedArray('another data name', function () {
  return open('./0.5K_jp-rifat_Shipping.csv')
    .split('\n')
...
export const options = {
  scenarios: {
    default: {
      ...
      vus: 1,
      iterations: 1, // Increase the total test duration
    },
  },
  thresholds: {
    checks: ['rate==1.0'],
...
export default function () {
  group('Testing PT Console API', function () {
    ...
    const JWT_token = {
      headers: {
        Authorization:
          'Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0YW52aXIua2hhbkBiaml0Z3JvdXAuY29tIiwiZXhwIjoxNzI0MzE2MjA0LCJpYXQiOjE3MjQyMjk4MDR9.bSKtUCpLlhndp5sq3E1uhhF1peYrA5CCfzA7jPdAUqU'
      },
      timeout: '6000000ms'
    };

    const formData = {
      csv_file: http.file(payload, 'Up-loading 0.5K devices JP_VUs 1 using k6 testing tool', 'text/csv'),
    };
...
export function handleSummary(data) {
  ...
  return {
    [`${outputDirectory}/html_report_0.5K_devices_JP_VUs 1_${currentTime}.html`]: htmlReport(data),
  };
}
