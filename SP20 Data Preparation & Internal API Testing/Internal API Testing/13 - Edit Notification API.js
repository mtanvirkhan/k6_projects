...
export default function () {
  const url = 'https://cw-consolebo.global.qa.pocketalk.com/iotconsoleapi-global-bo/api/private/update-notification';

  const payload = JSON.stringify({
    server_region_list: [1],
    notification_id: 2807,
    destination_type: "Bell",
    notification_status: "DRAFTED",
    is_scheduled: false,
    bell_message_title: "Updated using k6 tool API testing",
  ...
  const result = check(res, {
    ...
    'message is OK': (r) => jsonResponse.message === 'OK' // Adjust based on actual expected message
  });
